...
bool ValueManager::exists(const UniqueKey *UK) {
  if (vinfos.find(UK) != vinfos.end()) return true;
  return false;
}

// ValueInformation * ValueManager::getValueInfo(Value *val, Type * ty, long
// num){
...
ValueInformation *ValueManager::getValueInfo(const UniqueKey *UK) {
  if (this->exists(UK)) return vinfos[UK];
  return NULL;
}
...
// void ValueManager::addValueInfo(Value * val, Type * memType, Type * parType,
// long num){
//     if(!this->exists(val, memType, num))
//         vinfos[UniqueKey(val, memType, num)] = new ValueInformation(val,
//         memType, parType, num);
//     return;
// }
